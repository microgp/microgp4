...
class EvaluatorABC(ABC):
    ...
    *   **strip_phenotypes**: In all Evaluators, set option `strip_phenotypes` to convert the genotype into a
...
class PythonEvaluator(EvaluatorABC):
    ...
    *   process-based (based on `joblib` [2]_): Multiple functions are evaluated in different processes.
    ...
    Use option `strip_phenotypes` (see :py:class:`classes.evaluator.EvaluatorABC`) to convert the genotype into a
    single-line string.
...
class MakefileEvaluator(EvaluatorABC):
    ...
    The `MakefileEvaluator` helps the use of an external evaluator based on the `make` command. For each individual
    to be evaluated: it creates a temporary directory; puts in the directory the `Makefile` and all other necessary
    files; dumps the genome of the individual; enters the directory (chdir) and calls `make`; gets the result from
    the standard output; and eventually destroys the temporary directory.
